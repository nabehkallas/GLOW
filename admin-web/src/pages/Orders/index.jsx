...
export default function Orders() {
  ...
  const failOrder = async (id) => {
    ...
    await api.patch(`admin/orders/${id}/fail`)
    load()
  }

  const returnOrder = async (id) => {
    if (!confirm(t('orders.returnConfirm'))) return
    await api.patch(`admin/orders/${id}/return`)
    load()
  }

  const approveReturn = async (id) => {
    await api.patch(`admin/orders/${id}/approve-return`)
    load()
  }

  const denyReturn = async (id) => {
    await api.patch(`admin/orders/${id}/deny-return`)
    load()
  }

  const approveCancellation = async (id) => {
    await api.patch(`admin/orders/${id}/approve-cancellation`)
    load()
  }

  const denyCancellation = async (id) => {
    await api.patch(`admin/orders/${id}/deny-cancellation`)
    load()
  }
  ...
  return (
    <Layout>
      <div className="p-8 space-y-6">
        <h1 className="text-2xl font-bold text-prima-dark">{t('orders.title')}</h1>
          ...
          <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
            <table className="w-full text-sm">
              <thead className="bg-slate-50 text-prima-dark text-xs uppercase">
                <tr>
                  {[t('orders.table.order'), t('orders.table.salon'), t('orders.table.items'), t('orders.table.total'), t('orders.table.status'), t('common.actions')].map((h, i) => (
      ...
      <OrderDetailModal order={selected} type="b2b" onClose={() => setSelected(null)} />

      {editing && (
        <EditOrderModal
          order={editing}
          endpoint={`admin/orders/${editing.id}`}
