...
export default function Analytics() {
  ...
  const [psFrom, setPsFrom] = useState(firstOfMonthStr())
  const [psTo, setPsTo] = useState(todayStr())
  ...
  useEffect(() => {
    api.get('admin/analytics/products-sold', { params: { date_from: psFrom, date_to: psTo } })
      .then(({ data }) => setPsData(data))
  }, [psFrom, psTo])

  const clearPsRange = () => {
    setPsFrom(firstOfMonthStr())
    setPsTo(todayStr())
  }
        ...
        {!data ? (
          ...
          <div className="space-y-6">
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
              <StatCard label={t('analytics.totalRevenue')}      value={`$${totalRevenue}`} gradient />
              ...
              <StatCard label={t('analytics.pendingSalons')}     value={ov.pending_salons ?? '—'} accent />
            </div>
            ...
            {data.monthly_appointments?.length > 0 && (
              <ChartCard title={t('analytics.monthlyAppointments')}>
                <BarChart data={data.monthly_appointments}>
                  ...
                  <Tooltip />
                  <Bar dataKey="count" fill="url(#apptGrad)" radius={[4, 4, 0, 0]} name={t('analytics.seriesAppointments')} />
                </BarChart>
              </ChartCard>
            )}

            {data.monthly_orders?.length > 0 && (
              <ChartCard title={t('analytics.monthlyOrdersRevenue')}>
                <LineChart data={data.monthly_orders}>
                  <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#e5e7eb" />
                  <XAxis dataKey="month" tick={{ fontSize: 11 }} />
                  <YAxis yAxisId="left" tick={{ fontSize: 11 }} />
                  <YAxis yAxisId="right" orientation="right" tick={{ fontSize: 11 }} />
                  <Tooltip formatter={(v, name) => name === t('analytics.seriesRevenue') ? `$${v}` : v} />
                  <Legend />
                  <Line yAxisId="left"  type="monotone" dataKey="count"   stroke="#263238" strokeWidth={2} dot={false} name={t('analytics.seriesOrders')} />
                  <Line yAxisId="right" type="monotone" dataKey="revenue" stroke="#e8481c" strokeWidth={2} dot={false} name={t('analytics.seriesRevenue')} />
                </LineChart>
              </ChartCard>
            )}
            ...
            {data.monthly_registrations?.length > 0 && (
              <ChartCard title={t('analytics.newRegistrations')}>
                <BarChart data={data.monthly_registrations}>
                  ...
                  <Tooltip />
                  <Legend />
                  <Bar dataKey="clients" fill="#263238" radius={[4, 4, 0, 0]} name={t('analytics.seriesClients')} />
                  <Bar dataKey="salons"  fill="#e8481c" radius={[4, 4, 0, 0]} name={t('analytics.seriesSalons')} />
                </BarChart>
              </ChartCard>
            )}
            ...
            <div className="grid md:grid-cols-2 gap-6">
              {data.top_salons?.length > 0 && (
                <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
                  <div className="px-6 py-4 border-b border-gray-100 font-semibold text-prima-dark">{t('analytics.topSalons')}</div>
                  <table className="w-full text-sm">
                    <thead className="bg-slate-50 text-prima-dark text-xs uppercase">
                      <tr>
                        {[t('analytics.table.salon'), t('analytics.table.city'), t('analytics.table.bookings'), t('analytics.table.revenue')].map((h, i) => (
              ...
              {data.top_products?.length > 0 && (
                <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
                  <div className="px-6 py-4 border-b border-gray-100 font-semibold text-prima-dark">{t('analytics.topProducts')}</div>
                  <table className="w-full text-sm">
                    <thead className="bg-slate-50 text-prima-dark text-xs uppercase">
                      <tr>
                        {[t('analytics.table.product'), t('analytics.table.ordered'), t('analytics.table.revenue')].map((h, i) => (
                          <th key={i} className="px-4 py-3 text-start">{h}</th>
                        ))}
                      </tr>
                    </thead>
                      ...
                      {data.top_products.map((p) => (
                        <tr key={p.id} className="border-t border-gray-100 hover:bg-slate-50/50">
                          <td className="px-4 py-3">
                            ...
                            {p.category_en && <p className="text-xs text-orange-500">{p.category_en}</p>}
                          </td>
            ...
            <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
              <div className="px-6 py-4 border-b border-gray-100 flex flex-wrap items-center justify-between gap-3">
                <span className="font-semibold text-prima-dark">{t('analytics.productsSold')}</span>
                <div className="flex items-center gap-2 text-sm">
                  <label className="text-gray-500">{t('analytics.dateFrom')}</label>
                  <input
                    type="date"
                    value={psFrom}
                    max={psTo}
                  ...
                  />
                  <label className="text-gray-500">{t('analytics.dateTo')}</label>
                  <input
                    type="date"
                    value={psTo}
                    min={psFrom}
                  ...
                  <button
                    type="button"
                    onClick={clearPsRange}
                    className="text-xs text-prima-orange hover:underline"
                  >
                    {t('analytics.clearDates')}
                  </button>
                </div>
              </div>
              {!psData ? (
                <p className="text-gray-400 text-sm px-6 py-6">{t('common.loading')}</p>
              ) : psData.products.length === 0 ? (
                <p className="text-gray-400 text-sm px-6 py-6">{t('common.noResults')}</p>
              ) : (
                <table className="w-full text-sm">
                  <thead className="bg-slate-50 text-prima-dark text-xs uppercase">
                    <tr>
                      {[t('analytics.table.product'), t('analytics.table.quantitySold')].map((h, i) => (
                        <th key={i} className="px-4 py-3 text-start">{h}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                    {psData.products.map((p) => (
                      <tr key={p.id} className="border-t border-gray-100 hover:bg-slate-50/50">
                        <td className="px-4 py-3">
                          <p className="font-semibold text-prima-dark">{p.name}</p>
                          {p.category_en && <p className="text-xs text-orange-500">{p.category_en}</p>}
                        </td>
                        <td className="px-4 py-3 font-medium">{p.quantity_sold}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
