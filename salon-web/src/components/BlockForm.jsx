...
export default function BlockForm({ onClose, onSaved, defaultScheduledAt }) {
  const { t } = useTranslation()
  const { register, handleSubmit, formState: { errors, isSubmitting } } = useForm({
    defaultValues: { scheduled_at: defaultScheduledAt ?? '', duration_minutes: '60' },
  })
  ...
  const onSubmit = async (data) => {
    ...
    try {
      await api.post('/salon/schedule-blocks', {
        ...
        salon_service_id: data.salon_service_id ? Number(data.salon_service_id) : undefined,
        note: data.note || undefined,
      })
      onSaved()
    } catch (e) {
      setError(e.response?.data?.message ?? t('workingHours.grid.blockFailed'))
    }
  }

  return (
    <div className="p-4 sm:p-6">
      <h2 className="font-semibold text-prima-dark mb-4">{t('workingHours.grid.blockTitle')}</h2>
      <form onSubmit={handleSubmit(onSubmit)} className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <Field label={t('appointments.duration')} error={errors.duration_minutes?.message}>
          <select {...register('duration_minutes', { required: true })} className={inp}>
            {DURATION_OPTIONS.map((m) => (
              <option key={m} value={m}>{formatDuration(m)}</option>
            ))}
          </select>
        </Field>
        <Field label={t('workingHours.grid.blockScope')}>
          <select {...register('salon_service_id')} className={inp}>
            <option value="">{t('workingHours.grid.blockWholeSalon')}</option>
            {services.map((s) => (
        ...
        <Field label={`${t('common.notes')} (${t('common.optional')})`}>
          <input {...register('note')} className={inp} placeholder={t('workingHours.grid.blockNotePlaceholder')} />
        </Field>
        ...
        <div className="col-span-full flex gap-3">
          <button
            ...
            className="px-4 py-2 bg-prima-dark hover:bg-slate-700 text-white text-sm rounded-lg disabled:opacity-50 font-medium shadow-sm transition-colors"
          >
            {isSubmitting ? t('common.saving') : t('workingHours.grid.blockSubmit')}
          </button>
